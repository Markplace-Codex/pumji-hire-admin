...
@Component({
  selector: 'app-earn-more-interviewers-page',
  imports: [RouterLink],
  templateUrl: './earn-more-interviewers-page.component.html',
  styleUrl: './earn-more-interviewers-page.component.scss',
})
export class EarnMoreInterviewersPageComponent {
  private readonly http = inject(HttpClient);
  private readonly pageSize = 10;

  protected readonly isLoading = signal(true);
  protected readonly errorMessage = signal<string | null>(null);
  protected readonly interviewers = signal<InterviewerItem[]>([]);
  ...
  private load(pageIndex: number): void {
    ...
    this.http.get<ApiResponse>(
      `${resolveApiBasePath()}/api/SuperAdmin/GetAllInterviewerRequests`,
      { params: { pageIndex, pageSize: this.pageSize } }
    ).subscribe({
      next: (res) => {
        const pagination = res.paginationInterviewerList?.pagination;
        const list = res.paginationInterviewerList?.interviewer ?? [];
        this.interviewers.set(list);
